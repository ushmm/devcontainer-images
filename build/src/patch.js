...
async function patchImage(
  ...
  console.log(
    `(*) Tags to update: ${JSON.stringify(
      repoAndTagList.reduce((prev, repoAndTag) => {
        return prev + repoAndTag.repository + ":" + repoAndTag.tag + " ";
      }, ""),
      undefined,
      4,
    )}`,
  ...
  const tagArgs = repoAndTagList.reduce((prev, repoAndTag) => {
    return prev.concat([
      "--tag",
      `${registry}/${repoAndTag.repository}:${repoAndTag.tag}`,
    ]);
  }, []);
    ...
    try {
      await asyncUtils.spawn(
        ...
        [
          ...
          .concat(tagArgs)
          .concat("-f", dockerFilePath, patchPath),
        spawnOpts,
      );
    } catch (ex) {
      ...
      if (
        ex.result &&
        ex.result.indexOf("no space left on device") >= 0 &&
        retry === false
      ) {
  ...
  await asyncUtils.forEach(repoAndTagList, async (repoAndTag) => {
    await asyncUtils.spawn(
      "docker",
      ["push", `${registry}/${repoAndTag.repository}:${repoAndTag.tag}`],
      spawnOpts,
    );
  });
...
async function deleteUntaggedImages(imageIds, registry) {
  ...
  console.log(
    `(*) Manifests to delete: ${JSON.stringify(manifests, undefined, 4)}`,
  );
  ...
  await asyncUtils.forEach(manifests, async (manifest) => {
    if (manifest.tags.length > 0) {
      console.log(
        `(!) Skipping ${manifest.digest} because it has tags: ${manifest.tags}`,
      );
      return;
    }
...
async function getImageRepositoryAndTags(imageId, registry) {
  ...
  const repositoryListOutput = await asyncUtils.spawn(
    "az",
    [
      "acr",
      "repository",
  ...
  await asyncUtils.forEach(repositoryList, async (repository) => {
    ...
    repoAndTagList = repoAndTagList.concat(
...
async function getImageManifests(imageIds, registry) {
  ...
  await asyncUtils.forEach(repositoryList, async (repository) => {
    ...
    const registryManifestListOutput = await asyncUtils.spawn(
      "az",
      [
        "acr",
        "repository",
...
async function patchAll(registry, registryPath) {
  ...
  const patchStatus = (await asyncUtils.exists(patchStatusFilePath))
    ? await jsonc.read(patchStatusFilePath)
    : { complete: {}, failed: {} };
  patchStatus.failed = {};
  const patchList = await asyncUtils.readdir(patchRoot, {
    withFileTypes: true,
  });
  await asyncUtils.forEach(patchList, async (patchEntry) => {
    ...
    if (patchEntry.isDirectory()) {
      try {
        await patch(
          path.join(patchRoot, patchEntry.name),
          registry,
          registryPath,
        );
        ...
        await asyncUtils.writeFile(
          patchStatusFilePath,
          JSON.stringify(patchStatus, undefined, 4),
        );
        throw ex;
      }
    }
...
export { patchAll, patch, deleteUnpatchedImages };
