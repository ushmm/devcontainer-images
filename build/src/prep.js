...
const imageLabelPrefix = configUtils.getConfig(
  "imageLabelPrefix",
  "dev.containers",
);

// Prepares dockerfile for building or packaging
async function prepDockerFile(
  devContainerDockerfilePath,
  definitionId,
  repo,
  release,
  registry,
  ...
  // Read Dockerfile
  const devContainerDockerfileRaw = await asyncUtils.readFile(
    devContainerDockerfilePath,
  );
  // Use exact version of building, MAJOR if not
  const version = isForBuild
    ? configUtils.getVersionFromRelease(release, definitionId)
    : configUtils.majorFromRelease(release, definitionId);
  ...
  const prepResult = {
    ...
    devContainerDockerfileModified: await updateScriptSources(
      devContainerDockerfileRaw,
      repo,
      release,
      true,
    ),
  ...
  if (isForBuild) {
    if (prepResult.meta) {
      // Write meta.env
      const metaEnvTemplate = handlebars.compile(
        await asyncUtils.readFile(
          path.join(__dirname, "..", "assets", "meta.env"),
        ),
      );
      await asyncUtils.writeFile(
        path.join(devContainerJsonPath, "meta.env"),
        metaEnvTemplate(prepResult.meta),
      );
    ...
    const parentTag = configUtils.getParentTagForVersion(
      definitionId,
      version,
      registry,
      registryPath,
    ...
    if (parentTag) {
      prepResult.devContainerDockerfileModified = replaceFrom(
        prepResult.devContainerDockerfileModified,
        `FROM ${parentTag}`,
      );
    }

    prepResult.shouldFlattenBaseImage =
      configUtils.shouldFlattenDefinitionBaseImage(definitionId);
    if (prepResult.shouldFlattenBaseImage) {
      // Determine base image
      const baseImageFromCaptureGroups = /FROM\s+(.+):([^\s\n]+)?/.exec(
        prepResult.devContainerDockerfileModified,
      );
      let registryPath = baseImageFromCaptureGroups[1]
        .replace("${VARIANT}", variant)
        .replace("$VARIANT", variant);
      const tagName =
        baseImageFromCaptureGroups.length > 2
          ? baseImageFromCaptureGroups[2]
              .replace("${VARIANT}", variant)
              .replace("$VARIANT", variant)
          : null;
      ...
      prepResult.devContainerDockerfileModified = replaceFrom(
        prepResult.devContainerDockerfileModified,
        `FROM ${prepResult.flattenedBaseImageTag}`,
      );
    }
    ...
    const expectedRegistry = configUtils.getConfig(
      "stubRegistry",
      "mcr.microsoft.com",
    );
    const expectedRegistryPath = configUtils.getConfig(
      "stubRegistryPath",
      "devcontainers",
    );
    const fromCaptureGroups = new RegExp(
      `FROM\\s+(${expectedRegistry}/${expectedRegistryPath}/.+:.+)`,
    ...
    if (fromCaptureGroups && fromCaptureGroups.length > 0) {
      ...
      prepResult.devContainerDockerfileModified =
        prepResult.devContainerDockerfileModified.replace(
          fromCaptureGroups[0],
          `FROM ${fromDefinitionTag}`,
        );
    }
  }
...
async function createStub(
  ...
  const templateDockerfile = await configUtils.objectByDefinitionLinuxDistro(
    definitionId,
    stubPromises,
  );
  const userDockerFile = await processStub(
    templateDockerfile,
    definitionId,
    repo,
    release,
...
async function updateStub(
  ...
  const userDockerFileModified = await processStub(
    userDockerFile,
    definitionId,
    repo,
    release,
...
async function processStub(
  ...
  const devContainerImageVersion = configUtils.majorFromRelease(
    release,
    definitionId,
  );
  ...
  if (
    /ARG\s+VARIANT\s*=/.exec(userDockerFile) !== null &&
    configUtils.getVariants(definitionId) != null
  ) {
    const variant = configUtils.getVariants(definitionId)[0];
    const tagWithVariant = configUtils.getTagsForVersion(
      definitionId,
      devContainerImageVersion,
      registry,
      registryPath,
      "${VARIANT}",
    )[0];
    // Handle scenario where "# [Choice]" comment exists
    const choiceCaptureGroup = /(#\s+\[Choice\].+\n)ARG\s+VARIANT\s*=/.exec(
      userDockerFile,
    );
...
async function updateConfigForRelease(
  definitionId,
  repo,
  release,
  registry,
...
async function updateScriptSources(
  devContainerDockerfileRaw,
  repo,
  release,
  updateScriptSha,
  ...
  updateScriptSha =
    typeof updateScriptSha === "undefined" ? true : updateScriptSha;
  let devContainerDockerfileModified = devContainerDockerfileRaw;

  const scriptArgs =
    /ARG\s+.+_SCRIPT_SOURCE/.exec(devContainerDockerfileRaw) || [];
  await asyncUtils.forEach(scriptArgs, async (scriptArg) => {
    // Replace script URL and generate SHA if applicable
    const scriptCaptureGroups = new RegExp(
      `${scriptArg}\\s*=\\s*"(.+)/${scriptLibraryPathInRepo.replace(".", "\\.")}/(.+)"`,
    ).exec(devContainerDockerfileModified);
    if (scriptCaptureGroups) {
      ...
      devContainerDockerfileModified = devContainerDockerfileModified
        .replace(
          new RegExp(`${scriptArg}\\s*=\\s*".+"`),
          `${scriptArg}="${scriptSource}"`,
        )
        .replace(
          new RegExp(`${shaArg}\\s*=\\s*".+"`),
          `${shaArg}="${updateScriptSha ? sha : "dev-mode"}"`,
        );
    }
  });
...
function addLabels(prepResult) {
  ...
  let dockerFileContentsWithLabels =
    prepResult.devContainerDockerfileModified +
    "\n" +
    versionLabel +
    idLabel +
...
function addBuildArguments(prepResult) {
  const buildSettings = configUtils.getBuildSettings(
    prepResult.meta.definitionId,
  );

  for (let buildArg in buildSettings.buildArgs || {}) {
    prepResult.devContainerDockerfileModified =
      `ARG ${buildArg}="${buildSettings.buildArgs[buildArg]}"\n` +
      prepResult.devContainerDockerfileModified;
  }

  if (buildSettings.variantBuildArgs) {
    for (let buildArg in buildSettings.variantBuildArgs[
      prepResult.meta.variant
    ] || {}) {
...
export { createStub, updateStub, updateConfigForRelease, prepDockerFile };
