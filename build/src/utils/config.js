...
async function loadConfig(repoPath) {
  ...
  const imageBuildConfigFile = getConfig(
  ...
  const definitions = await asyncUtils.readdir(containersPath, {
    withFileTypes: true,
  });
  await asyncUtils.forEach(definitions, async (definitionFolder) => {
    ...
    const definitionPath = path.resolve(
      path.join(containersPath, definitionId),
    );
  ...
  for (let definitionId in config.definitionBuildSettings) {
    ...
    dependencies.imageVariants = definitionVariants
      ? definitionVariants.map((variant) =>
          dependencies.image.replace("${VARIANT}", variant),
        )
      : [dependencies.image];
    ...
    if (buildSettings.tags) {
      ...
      const variants = definitionVariants
        ? ["${VARIANT}", "$VARIANT"].concat(definitionVariants)
        : [undefined];

      variants.forEach((variant) => {
        const blankTagList = getTagsForVersion(
          definitionId,
          "",
          "ANY",
          "ANY",
        ...
        const devTagList = getTagsForVersion(
          definitionId,
          "dev",
          "ANY",
          "ANY",
...
function getDefinitionPath(definitionId, relative) {
  if (!allDefinitionPaths[definitionId]) {
    ...
    throw new Error(
      `Invalid definition ID '${definitionId}'. Available definition IDs are: ${availableIds}`,
    );
  }
  return relative
    ? allDefinitionPaths[definitionId].relativeToRootPath
    : allDefinitionPaths[definitionId].path;
}
...
function getLatestTag(definitionId, registry, registryPath) {
  ...
  return config.definitionBuildSettings[definitionId].tags.reduce(
    (list, tag) => {
...
function getTagsForVersion(
  ...
  if (version === "dev") {
    version = config.definitionBuildSettings[definitionId].versionedTagsOnly
      ? `dev-${definitionId.replace(/-/gm, "")}`
      : "dev";
  }
...
function getTagList(
  ...
  if (
    updateUnversionedTags &&
    !config.definitionBuildSettings[definitionId].versionedTagsOnly
  ) {
  ...
  const definitionLatestProperty =
...
function getSortedDefinitionBuildList(page, pageTotal, definitionsToSkip) {
      ...
      if (definitionsToSkip.indexOf(definitionId) < 0) {
        ...
        if (parentId) {
          ...
          if (typeof parentId !== "string") {
            parentId = createMultiParentBucket(
              parentId,
              parentBuckets,
              dupeBuckets,
            );
    ...
    if (definitionBucket) {
      definitionBucket.reverse().forEach((definitionId) => {
        ...
        if (!parentId && variants) {
          variants.forEach((variant) => {
            const skipVariant = skipParentVariants.filter(
              (item) => item.id === definitionId && item.variant === variant,
            );
          ...
          if (variants) {
            variants.forEach((variant) => {
              const variantId =
                config.definitionBuildSettings[definitionId].idMismatch ===
                  "true" && variant.includes("-")
                  ? variant.split("-")[1]
                  : variant;
          ...
          for (const id in parentId) {
            ...
            if (commonVariant) {
              ...
              if (parentVariants.includes(commonVariant)) {
                const parentItem = getDefinitionObject(
                  parentObjId,
                  commonVariant,
                );
                const childItem = getDefinitionObject(
                  definitionId,
                  commonVariant,
                );
                addToVariantsList(parentItem, childItem);
  ...
  console.log(
    `(*) Builds pagination needs at least ${variantsList.length} pages to parallelize jobs efficiently.\n`,
  );

  if (allPages.length > pageTotal) {
    // If too many pages, add extra pages to last one
    console.log(
      `(!) Not enough pages to for target page size. Adding excess definitions to last page.`,
    );
    let i = pageTotal;
  ...
  console.log(
    `(*) Builds paginated as follows: ${JSON.stringify(allPages, null, 4)}\n(*) Processing page ${page} of ${pageTotal}.\n`,
  );
...
function addToVariantsList(parentItem, childItem) {
  ...
  for (let x = 0; x < variantsList.length; x++) {
    for (let y = 0; y < variantsList[x].length; y++) {
      ...
      if (
        variantItem.id === parentItem.id &&
        variantItem.variant === parentItem.variant
      ) {
  ...
  if (!isAdded) {
    const item = [parentItem, childItem];
  ...
  for (let currentVariant in variantParentMap) {
    ...
    if (currentParentId !== parentId) {
      ...
      if (currentParentBucket && dupeBuckets.indexOf(currentParentId) < 0) {
        currentParentBucket.forEach((current) =>
          firstParentBucket.push(current),
        );
...
function getParentTagForVersion(
  ...
  if (parentId) {
    if (typeof parentId !== "string") {
      ...
      parentId = variant
        ? parentId[variant]
        : parentId[Object.keys(parentId)[0]];
    }
    ...
    if (parentVariantList) {
      ...
      let parentVariant =
        config.definitionBuildSettings[definitionId].parentVariant || variant;
      if (typeof parentVariant !== "string") {
        ...
        parentVariant = variant
          ? parentVariant[variant]
          : parentVariant[Object.keys(parentId)[0]];
      }
      parentVariantId =
        config.definitionBuildSettings[definitionId].idMismatch === "true" &&
        variant.includes("-")
          ? variant.split("-")[1]
          : variant;
...
function getUpdatedTag(
  ...
  const definition = getDefinitionFromTag(
    currentTag,
    currentRegistry,
    currentRegistryPath,
  );
  ...
  if (!definition) {
    const repository = new RegExp(
      `${currentRegistry}/${currentRegistryPath}/(.+):`,
    ).exec(currentTag)[1];
    const updatedTag = currentTag.replace(
      new RegExp(
        `${currentRegistry}/${currentRegistryPath}/${repository}:(dev-|${updatedVersion}-)?`,
      ),
      `${updatedRegistry}/${updatedRegistryPath}/${repository}:${updatedVersion}-`,
    );
    console.log(
      `    Using RegEx to update ${currentTag}\n    to ${updatedTag}`,
    );
    return updatedTag;
  }
  ...
  const updatedTags = getTagsForVersion(
    definition.id,
    updatedVersion,
    updatedRegistry,
    updatedRegistryPath,
...
function getDefinitionFromTag(tag, registry, registryPath) {
  ...
  const captureGroups = new RegExp(
...
function getFallbackPoolUrl(pkg) {
  const poolUrl = config.poolUrlFallback[pkg];
  console.log(`(*) Fallback pool URL for ${pkg} is ${poolUrl}`);
  return poolUrl;
}
...
export {
  ...
  getTagList,
  getVariants,
  getAllDefinitionPaths,
  getBuildSettings,
  getDefinitionFromTag,
  getDefinitionPath,
  getSortedDefinitionBuildList,
  getParentTagForVersion,
  getUpdatedTag,
  majorFromRelease,
  objectByDefinitionLinuxDistro,
  getDefinitionDependencies,
  getAllDependencies,
  getDefaultDependencies,
  getStagingFolder,
  getLinuxDistroForDefinition,
  getVersionFromRelease,
  getTagsForVersion,
  getFallbackPoolUrl,
  getPoolKeyForPoolUrl,
  getConfig,
  shouldFlattenDefinitionBaseImage,
  getDefinitionList,
};
