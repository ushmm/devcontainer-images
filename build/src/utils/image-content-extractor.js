...
async function getLinuxDistroInfo(imageTagOrContainerName) {
  const info = {};
  const osInfoCommandOutput = await getCommandOutputFromContainer(
...
async function getLinuxPackageInfo(
  ...
  const defaultPackages =
    configUtils.getDefaultDependencies(packageManager) || [];
  ...
  const packageVersionListOutput = await getCommandOutputFromContainer(
    imageTagOrContainerName,
    extractionConfig.listCommand +
      packageListCommandPart +
  ...
  const packageUriCommandOutput = await getCommandOutputFromContainer(
    imageTagOrContainerName,
    extractionConfig.getUriCommand +
      packageListCommandPart +
      " || echo 'Some packages were not found.'",
    true,
  );
  ...
  packageVersionList.forEach((packageVersion) => {
    ...
    if (packageVersion !== "") {
      const versionCaptureGroup = new RegExp(extractionConfig.lineRegEx).exec(
        packageVersion,
      );
      if (!versionCaptureGroup) {
        if (packageVersion === "Some packages were not found.") {
          ...
          console.log(
            `(!) Warning: Unable to parse output "${packageVersion}" - skipping.`,
          );
        }
        return;
      }
      const [, pkg, version] = versionCaptureGroup;
      const packageSettings = settings[pkg] || {};
      const cgIgnore =
        typeof packageSettings.cgIgnore === "undefined"
          ? true
          : packageSettings.cgIgnore; // default to true
      const poolUrl = getPoolUrlFromPackageVersionListOutput(
        packageUriCommandOutput,
        extractionConfig,
        pkg,
        version,
      );
...
function getPoolUrlFromPackageVersionListOutput(
  packageUriCommandOutput,
  config,
  pkg,
  version,
  ...
  const sanitizedPackage = pkg.replace(/\+/g, "\\+").replace(/\./g, "\\.");
  const sanitizedVersion = version
    .replace(/\+/g, "\\+")
    .replace(/\./g, "\\.")
    .replace(/:/g, "%3a");
  const uriCaptureGroup = new RegExp(
    config.poolUriMatchRegEx
      .replace("${PACKAGE}", sanitizedPackage)
      .replace("${VERSION}", sanitizedVersion),
    "m",
  ).exec(packageUriCommandOutput);

  if (!uriCaptureGroup) {
    const fallbackPoolUrl = configUtils.getFallbackPoolUrl(pkg);
    if (fallbackPoolUrl) {
      return fallbackPoolUrl;
    }
    console.log(`(!) No URI found for ${pkg} ${version}.`);
    return null;
  }
...
async function getNpmGlobalPackageInfo(imageTagOrContainerName, packageList) {
  ...
  console.log(
    `(*) Gathering information about globally installed npm packages...`,
  );

  const packageListString = packageList.reduce(
    (prev, current) => prev + " " + current,
    "",
  );
  const npmOutputRaw = await getCommandOutputFromContainer(
    imageTagOrContainerName,
    `bash -l -c 'set -e && npm ls --global --depth 1 --json ${packageListString}' 2>/dev/null`,
  );
  const npmOutput = JSON.parse(npmOutputRaw);

  return packageList.map((pkg) => {
    let packageJson = npmOutput.dependencies[pkg];
    if (!packageJson) {
      ...
      for (let packageInNpmOutput in npmOutput.dependencies) {
        const packageDependencies =
          npmOutput.dependencies[packageInNpmOutput].dependencies;
        if (packageDependencies) {
          packageJson = packageDependencies[pkg];
          if (packageJson) {
            break;
          }
        }
    ...
    if (!packageJson || !packageJson.version) {
      throw new Error(
        `Unable to parse version for ${pkg} from npm ls output: ${npmOutputRaw}`,
      );
    }
    return {
      name: pkg,
...
*/
async function getPipPackageInfo(
  imageTagOrContainerName,
  packageList,
  usePipx,
  imageId,
  ...
  packageList = packageList || [];
  const defaultPackages =
    configUtils.getDefaultDependencies(usePipx ? "pipx" : "pip") || [];
  packageList = defaultPackages.concat(packageList);
  ...
  const versionLookup = usePipx
    ? await getPipxVersionLookup(imageTagOrContainerName)
    : await getPipVersionLookup(imageTagOrContainerName, imageId);

  return packageList.map((pkg) => {
    return {
      name: pkg,
      version: versionLookup[pkg],
    };
  });
}
...
async function getGitRepositoryInfo(imageTagOrContainerName, gitRepos) {
  ...
  for (let repoName in gitRepos) {
    ...
    if (typeof repoPath === "string") {
      console.log(
        `(*) Getting remote and commit for ${repoName} at ${repoPath}...`,
      );
      // Go to the specified folder, see if the commands have already been run, if not run them and get output
      const remoteAndCommitOutput = await getCommandOutputFromContainer(
        imageTagOrContainerName,
        `git config --global --add safe.directory \\"${repoPath}\\" && cd \\"${repoPath}\\" && if [ -f \\".git-remote-and-commit\\" ]; then cat .git-remote-and-commit; else git remote get-url origin && git log -n 1 --pretty=format:%H -- . | tee /dev/null; fi`,
        true,
      );
...
async function getOtherComponentInfo(
  ...
  const componentList = [];
  for (let otherName in otherComponents) {
    const otherSettings = mergeOtherDefaultSettings(
      otherName,
      otherComponents[otherName],
    );
    if (typeof otherSettings === "object") {
      console.log(`(*) Getting version for ${otherName}...`);
      // Run specified command to get the version number
      const otherVersion = await getCommandOutputFromContainer(
        imageTagOrContainerName,
        otherSettings.versionCommand,
        false,
        getUserName(imageId),
      );
      componentList.push({
...
function mergeOtherDefaultSettings(otherName, settings) {
  const otherDefaultSettings = configUtils.getConfig(
...
async function getGemPackageInfo(imageTagOrContainerName, packageList) {
  ...
  const gemListOutput = await getCommandOutputFromContainer(
    imageTagOrContainerName,
    "bash -l -c 'set -e && gem list -d --local' 2>/dev/null",
  );
  return packageList.map((gem) => {
    const gemVersionCaptureGroup = new RegExp(
      `^${gem}\\s\\(([^\\),]+)`,
      "m",
    ).exec(gemListOutput);
...
async function getCargoPackageInfo(imageTagOrContainerName, packages) {
  ...
  for (let crate in packages) {
    if (typeof crate === "string") {
      ...
      const versionOutput = await getCommandOutputFromContainer(
        imageTagOrContainerName,
        versionCommand,
      );
      const crateVersionCaptureGroup = new RegExp(
        "[0-9]+\\.[0-9]+\\.[0-9]+",
        "m",
      ).exec(versionOutput);
...
async function getGoPackageInfo(imageTagOrContainerName, packages) {
  ...
  const componentList = [];
  const packageInstallOutput = await getCommandOutputFromContainer(
    imageTagOrContainerName,
    "cat /usr/local/etc/vscode-dev-containers/go.log",
  );
  for (let pkg in packages) {
    if (typeof pkg === "string") {
      const versionCommand = packages[pkg];
      let version;
      if (versionCommand) {
        version = await getCommandOutputFromContainer(
          imageTagOrContainerName,
          versionCommand,
        );
      } else {
        const versionCaptureGroup = new RegExp(
          `downloading\\s*${pkg}\\s*v([0-9]+\\.[0-9]+\\.[0-9]+.*)\\n`,
...
async function getImageInfo(imageTagOrContainerName) {
  ...
  if (isContainerName(imageTagOrContainerName)) {
    image = await asyncUtils.spawn(
      "docker",
      ["inspect", "--format='{{.Image}}'", imageTagOrContainerName.trim()],
  ...
  let name, digest;
  try {
    const imageNameAndDigest = await asyncUtils.spawn(
      "docker",
  ...
  const nonRootUser = await getCommandOutputFromContainer(
    imageTagOrContainerName,
    "id -un 1000",
    true,
  );
...
async function removeProcessingContainer(containerName) {
  await asyncUtils.spawn("docker", ["rm", "-f", containerName], {
    shell: true,
    stdio: "inherit",
  });
}
...
async function getCommandOutputFromContainer(
  imageTagOrContainerName,
  command,
  forceRoot,
  userName,
  ...
  const runArgs = isContainerName(imageTagOrContainerName)
    ? ["exec"].concat(
        userName ? ["-u", userName] : forceRoot ? ["-u", "root"] : [],
      )
    : ["run", "--init", "--privileged", "--rm"].concat(
        forceRoot ? ["-u", "root"] : [],
      );
  ...
  const result = await asyncUtils.spawn("docker", runArgs, {
    shell: true,
    stdio: "pipe",
  });
  // Filter out noise from ENTRYPOINT output
  const filteredResult = result.substring(
    result.indexOf("~~~BEGIN~~~") + 11,
    result.indexOf("~~~END~~~"),
  );
...
async function getAllContentInfo(imageTag, dependencies, imageId) {
  ...
  try {
    ...
    const contents = {
      ...
      distro: distroInfo,
      linux: await getLinuxPackageInfo(
        containerName,
        getLinuxPackageManagerDependencies(dependencies, distroInfo),
        distroInfo,
      ),
      npm: await getNpmGlobalPackageInfo(containerName, dependencies.npm),
      pip: await getPipPackageInfo(
        containerName,
        dependencies.pip,
        false,
        imageId,
      ...
      other: await getOtherComponentInfo(
        containerName,
        dependencies.other,
        "other",
      ),
      languages: await getOtherComponentInfo(
        containerName,
