...
async function push(
  ...
  replaceImages =
    configUtils.getVersionFromRelease(release, definitionId) == "dev" ||
    replaceImages;
  ...
  if (definitionId) {
    ...
    const definitionsToPush = configUtils.getSortedDefinitionBuildList(
      page,
      pageTotal,
      definitionsToSkip,
    );
    await asyncUtils.forEach(definitionsToPush, async (currentJob) => {
      ...
      console.log(
        `**** Pushing ${currentJob["id"]}: ${currentJob["variant"]} ${release} ****`,
      );
      await pushImage(
        currentJob["id"],
        currentJob["variant"] || null,
        repo,
        release,
        updateLatest,
        registry,
...
async function pushImage(
  ...
  console.log(
    `(*) Prep Dockerfile for ${definitionId} ${variant ? 'variant "' + variant + '"' : ""}...`,
  );
  const prepResult = await prep.prepDockerFile(
    dockerFilePath,
    definitionId,
    repo,
    release,
  ...
  if (prepOnly) {
    ...
    if (prepResult.shouldFlattenBaseImage) {
      ...
      await flattenBaseImage(
        prepResult.baseImageTag,
        prepResult.flattenedBaseImageTag,
        pushImages,
      );
    }
    ...
    const imageNamesWithVersionTags = configUtils.getTagList(
      definitionId,
      release,
      updateLatest,
      registry,
    ...
    const secondaryImageNamesWithVersionTags = configUtils.getTagList(
      definitionId,
      release,
      updateLatest,
      registry,
    ...
    console.log(
      `(*) Tags:${imageNamesWithVersionTags.reduce((prev, current) => (prev += `\n     ${current}`), "")}`,
    );
    console.log(
      `(*) Secondary Tags:${secondaryImageNamesWithVersionTags.reduce((prev, current) => (prev += `\n     ${current}`), "")}`,
    );
    ...
    console.log(
      `(*) Target image architectures: ${architectures.reduce((prev, current) => (prev += `\n     ${current}`), "")}`,
    );
    let localArchitecture = process.arch;
    switch (localArchitecture) {
      case "arm":
        localArchitecture = "linux/arm/v7";
        break;
      case "aarch32":
        localArchitecture = "linux/arm/v7";
        break;
      case "aarch64":
        localArchitecture = "linux/arm64";
        break;
      case "x64":
        localArchitecture = "linux/amd64";
        break;
      case "x32":
        localArchitecture = "linux/386";
        break;
      default:
        localArchitecture = `linux/${localArchitecture}`;
        break;
    }
    ...
    if (!pushImages) {
      console.log(
        `(*) Push disabled: Only building local architecture (${localArchitecture}).`,
      );
    }

    if (
      replaceImage ||
      !(await isDefinitionVersionAlreadyPublished(
        definitionId,
        release,
        registry,
        registryPath,
      ...
      const context = devContainerJson.build
        ? devContainerJson.build.context || "."
        : devContainerJson.context || ".";
      const workingDir = path.resolve(dotDevContainerPath, context);
      let imageNameParams = imageNamesWithVersionTags.reduce(
        (prev, current) => prev.concat(["--image-name", current]),
        [],
      );

      const secondaryImageNameParams =
        secondaryImageNamesWithVersionTags.reduce(
          (prev, current) => prev.concat(["--image-name", current]),
          [],
        );
...
async function flattenBaseImage(
  ...
  const flattenedImageCaptureGroups = /([^\/]+)\/(.+):(.+)/.exec(
    flattenedBaseImageTag,
  );
  if (
    await isImageAlreadyPublished(
      flattenedImageCaptureGroups[1],
      flattenedImageCaptureGroups[2],
      flattenedImageCaptureGroups[3],
    )
  ) {
  ...
  await asyncUtils.spawn(
    ...
    [
      ...
      "-d",
      "--name",
      "devcontainers-build-flatten",
      baseImageTag,
      "bash",
    ],
    processOpts,
  );
  const containerInspectOutput = await asyncUtils.spawn(
    "docker",
    ["inspect", "devcontainers-build-flatten"],
    { shell: true, stdio: "pipe" },
  );
  console.log("(*) Flattening (this could take a while)...");
  const config = JSON.parse(containerInspectOutput)[0].Config;
  const envString = config.Env.reduce(
    (prev, current) => prev + " " + current,
    "",
  );
  const importArgs = `-c 'ENV ${envString}' -c 'ENTRYPOINT ${JSON.stringify(config.Entrypoint)}' -c 'CMD ${JSON.stringify(config.Cmd)}'`;
  await asyncUtils.exec(
    `docker export devcontainers-build-flatten | docker import ${importArgs} - ${flattenedBaseImageTag}`,
    processOpts,
  );
  await asyncUtils.spawn(
    "docker",
    ["container", "rm", "-f", "devcontainers-build-flatten"],
    processOpts,
  );
...
async function isDefinitionVersionAlreadyPublished(
  definitionId,
  release,
  registry,
  registryPath,
  ...
  const tagsToCheck = configUtils.getTagList(
    definitionId,
    release,
    false,
    registry,
  ...
  return await isImageAlreadyPublished(
    registryName,
    tagParts[0].replace(/[^\/]+\//, ""),
    tagParts[1],
  );
...
async function isImageAlreadyPublished(registryName, repositoryName, tagName) {
  ...
  const repositoriesOutput = await asyncUtils.spawn(
    "az",
    [
      "acr",
      "repository",
  ...
  if (repositories.indexOf(repositoryName) < 0) {
    console.log(
      "(*) Repository does not exist. Image version has not been published yet.",
    );
    return false;
  }

  // Assuming repository exists, check if tag exists
  const tagListOutput = await asyncUtils.spawn(
    "az",
    [
      "acr",
      "repository",
